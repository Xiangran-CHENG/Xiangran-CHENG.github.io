...
- Home's research directions use `assets/research-directions-map.svg` and `assets/research-directions-map-mobile.svg` (selected at 760px and below). The transparent relationship map puts formation modeling within the central synthesizability question, connects structural targets and material states with structure-function research, and connects performance requirements and candidate materials with energy research. Scientific agents are a capability in development that supports all directions. Arrows represent shared information, not a demonstrated experimental loop across every example. Four links below the map open the corresponding Research sections. Update both SVG layouts together.
...
- Home's Our Research Strategy figure uses editable scientific SVGs: `assets/synthesis-science-transparent.svg` and the portrait `assets/synthesis-science-transparent-mobile.svg`. The diagram connects processing conditions, reacting interfaces and competing phase pathways with experimental observations, AI and physical modeling. Curves, observations and diffraction patterns are schematic, not research results. Update both layouts together; the portrait version is selected below 541px.
- The Home image viewer reads its image, title and caption from the selected figure, so each diagram and the broader research-philosophy illustration open with their own descriptions. Figures marked `data-artwork-theme="light"` use the light Mission background in the viewer. Native links still open the original assets when JavaScript is unavailable.
...
- Home's illustration viewer lives in `group/home.js`, with styles in `group/home.css`. Clicking a preview image opens an accessible native dialog; Zoom in reveals image details, and Close or Escape returns focus to the opening link. Without JavaScript, preview links open the original image directly. The source PNG and its colors are preserved.
