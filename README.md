...
- Home (`group/index.html`) leads with the vision "Make materials synthesis predictable" and mission "Understand formation. Guide synthesis." It connects these goals to three research commitments, selected publications and the team. Its page-specific styles live in `group/home.css`. Research (`group/research.html`), People (`group/people.html`) and Join us (`group/join.html`) are separate pages; edit their corresponding content there. All group pages are in English and do not read the personal homepage's saved language preference.
- Home's selected publications are work by Xiangran Cheng and collaborators, presented as research foundations. Their titles, DOIs and authorship roles must stay consistent with `group/publications.html`; each links to the full record. They are not presented as completed demonstrations of the group's synthesis-prediction vision. Keep goals and methods under development distinct from demonstrated results.
...
- The gallery contains four conceptual visuals/designs. Both personal photographs and the empty Photographs section were removed at the owner's request, pending replacement images. The original recruitment poster retains the earlier Materials Builders name.
