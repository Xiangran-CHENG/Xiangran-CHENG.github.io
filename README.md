...
- Home (`group/index.html`) leads with the vision "Build the materials we need." and mission "Understand formation. Guide synthesis." It connects societal needs to reliable synthesis and functional validation, with synthesizability as the core scientific question and three connected directions. A Big Picture section places the group in the processing–structure–properties–performance relationships of materials science, with forward prediction and inverse design linked by experimental feedback. The synthesis–structure connection is highlighted as the team's core focus. The positioning statement is "AI for predictive synthesis science." The synthesis-gap figure and team information ground that narrative. Its page-specific styles live in `group/home.css`. Research (`group/research.html`), People (`group/people.html`) and Join us (`group/join.html`) are separate pages; edit their corresponding content there. All group pages are in English and do not read the personal homepage's saved language preference.
- The strategy is "Observe formation. Test mechanisms. Guide synthesis." Progress is framed as a goal: predictions under new conditions, experimentally tested synthesis routes and verified function. Synthesizability remains the core question; scientific agents support evidence and research decisions across the four directions. The materials map is accessible HTML/CSS, with a vertical layout on small screens. The simplified relationships do not imply a unique inverse mapping or that synthesis is the sole bottleneck across materials science.
...
- `assets/materials-core-philosophy-2026.png` (1672 × 941) is an AI-generated conceptual illustration created for the group: societal needs, material targets, synthesis pathways and functional validation connected by experimental feedback. It is displayed uncropped as the main visual in Home's Big Picture section. The earlier HTML/CSS materials-science map remains available in an expandable explanation below it. The illustration expresses research goals, not measured results.
